...
const [filteredData, setFilteredData] = useState([]);
    ...
    useEffect(() => {
        // Check if filters are saved in local storage
        const savedFilters = localStorage.getItem('filters');
        if (savedFilters) {
            setSearchText(savedFilters);
        }
    ...
    const deleterecor = async(id)=>{
      ...
      useEffect(() => {
        ...
        const filteredResults = mydata.filter((item) =>{
           if(searchText==' '){
        return item;
           }else if(item.name.toLowerCase().includes(searchText.toLowerCase() || item.phone.includes(searchText)) || item.email.includes(searchText)){
        ...
        <div className='container page border'>
            <div className='row'>
                <div className='col-12'>
                    ...
                    <button type="button" onClick={sortAlphabeticaly}>A to B </button>
